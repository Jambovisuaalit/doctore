#!/usr/bin/env python3
"""Run the locked MLB training dataset through the complete Doctore decision chain."""
...
from production_artifact_pipeline import run_production_pipeline


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Validate a locked MLB training dataset, run expanding walk-forward "
            "XGBRegressor pricing and calibration, create canonical immutable model "
            "artifacts, and execute bet_decision_core."
        )
    )
